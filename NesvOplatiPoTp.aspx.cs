...
namespace Sbyt
{
    public partial class NesvOplatiReportNew : Page
    {
      ...
      //  String bytGuid = Guid.NewGuid().ToString();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if ((promTableNames.Count > 0) && (promTableNames.Count <= Constants.PromBytTablesCount))
            {
                ...
                for (int i = 0; i < promTableNames.Count; i++)
                {
                 ...
                 promTable = OracleOplati.Instance.GetOplatiTablePromTP(promTableNames, ConfigurationHelper.NesvOraConnectionStringBalPoTP, Request.QueryString["DOC_CODE"]);
              //  promTable =
              //  OracleOplati.Instance.PromGetOraTable(OracleOplati.Instance.FormSqlProm(promTableNames, Request.QueryString["DOC_CODE"]), ConfigurationHelper.NesvOraConnectionStringBalPoTP, Constants.NesvResLongName);
            ...
            if ((bytTableNames.Count > 0) && (bytTableNames.Count <= Constants.PromBytTablesCount))
            {
                ...
                bytTable = OracleOplati.Instance.GetOplatiTableBytTP(bytTableNames, ConfigurationHelper.NesvOraConnectionStringBalPoTP, Request.QueryString["DOC_NAME"]);
               // bytTable =
               // OracleOplati.Instance.BytGetOraTable(OracleOplati.Instance.FormSqlByt(bytTableNames, Request.QueryString["DOC_NAME"]), ConfigurationHelper.NesvOraConnectionStringBalPoTP, Constants.NesvResLongName);
        ...
        protected void PromHyper(object sender, EventArgs e)
        {
            ...
            Response.Clear();
            Response.AddHeader("Content-Disposition", "attachment; filename=" + "NesvProm" + Request.QueryString["DOC_NAME"] + ".xls");
            Response.Charset = "";
        ...
        protected void BytHyper(object sender, EventArgs e)
        {
            ...
            Response.Clear();
            Response.AddHeader("Content-Disposition", "attachment; filename=" + "NesvByt" + Request.QueryString["DOC_NAME"] + ".xls");
            Response.Charset = "";
            Response.ContentType = "application/vnd.ms-excel";
            Response.Write(sw.ToString());
            Response.Flush();
