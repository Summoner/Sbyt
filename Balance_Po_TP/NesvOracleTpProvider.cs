...
{
    public class NesvOracleTpProvider : BaseDal
    {
        ...
        #region Instance
        private NesvOracleTpProvider() { }

        [ThreadStatic]
        private static NesvOracleTpProvider _instance;

        public static NesvOracleTpProvider Instance
        {
            get { return _instance ?? (_instance = new NesvOracleTpProvider()); }
        }
        #endregion
        ...
        {
            using (OracleConnection connection = new OracleConnection(ConfigurationHelper.NesvPassportConnectionString))
            {
                if (TPsSort == "")
                {
                    TPsSort = "DOC_NAME";
                ...
                OracleCommand cmd =
                    new OracleCommand("Select DOC_CODE, DOC_NAME FROM (SELECT a.*, ROWNUM r FROM (SELECT DOC_CODE, DOC_NAME FROM PDOCS WHERE TEMPLATE_CODE = 'TP' ORDER BY DOC_NAME)a where rownum <= :HigerBound) where r >= :LowerBound order by " + TPsSort, connection);
        ...
        {
            using (OracleConnection connect = new OracleConnection(ConfigurationHelper.NesvPassportConnectionString))
            {
                OracleCommand command = new OracleCommand("SELECT COUNT(*) FROM PDOCS WHERE TEMPLATE_CODE = 'TP'", connect);
        ...
        {
            using (OracleConnection connection = new OracleConnection(ConfigurationHelper.NesvOraConnectionStringBalPoTP))
            {
