...
namespace Sbyt
{
    public partial class DergOplatiPoVL : Page
    {
      ...
      //  String bytGuid = Guid.NewGuid().ToString();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if ((promTableNames.Count > 0) && (promTableNames.Count <= Constants.PromBytTablesCount))
            {
                ...
                //Получаем список ТП
                List<string> listTpCodes = OracleOplati.Instance.GetListTPcodes(ConfigurationHelper.DergPassportConnectionString, Request.QueryString["DOC_CODE"]);

                
                promTable = Balance_Po_VL.OracleOplati.Instance.GetOplatiTablePromVl(promTableNames, ConfigurationHelper.DergOraConnectionStringBalPoTP, listTpCodes);
                    PromGridView.DataSource = promTable;
                    PromGridView.DataBind();
               //     Session["PromTable"] = promTable;
                  
              //  promTable =
               //  OracleOplati.Instance.PromGetOraTable(OracleOplati.Instance.FormSqlProm(promTableNames, listTpCodes), ConfigurationHelper.DergOraConnectionStringBalPoTP, Constants.DergResLongName);
              //  PromGridView.DataSource = promTable;
              //  PromGridView.DataBind();
            ...
            if ((bytTableNames.Count > 0) && (bytTableNames.Count <= Constants.PromBytTablesCount))
            {
                ...
                //Получаем список ТП
                List<string> listTpNames = OracleOplati.Instance.GetListTPnames(ConfigurationHelper.DergPassportConnectionString, Request.QueryString["DOC_CODE"]);

                 
                    bytTable = Balance_Po_VL.OracleOplati.Instance.GetOplatiTableBytVl(bytTableNames, ConfigurationHelper.DergOraConnectionStringBalPoTP, listTpNames);
                    BytGridView.DataSource = bytTable;
                    BytGridView.DataBind();
                  //  Session["BytTable"] = bytTable;
                  
             //   bytTable =
             //   OracleOplati.Instance.BytGetOraTable(OracleOplati.Instance.FormSqlByt(bytTableNames, listTpNames), ConfigurationHelper.DergOraConnectionStringBalPoTP, Constants.DergResLongName);
              //  BytGridView.DataSource = bytTable;
              //  BytGridView.DataBind();
        ...
        protected void PromHyper(object sender, EventArgs e)
        {
            ...
            Response.Clear();
            Response.AddHeader("Content-Disposition", "attachment; filename=" + "dergProm" + Request.QueryString["DOC_NAME_VL10"] + ".xls");
            Response.Charset = "";
        ...
        protected void BytHyper(object sender, EventArgs e)
        {
            ...
            Response.Clear();
            Response.AddHeader("Content-Disposition", "attachment; filename=" + "dergByt" + Request.QueryString["DOC_NAME_VL10"] + ".xls");
            Response.Charset = "";
            Response.ContentType = "application/vnd.ms-excel";
            Response.Write(sw.ToString());
            Response.Flush();
