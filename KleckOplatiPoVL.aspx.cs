...
namespace Sbyt
{
    public partial class KleckOplatiPoVL : Page
    {
     ...
     //   String bytGuid = Guid.NewGuid().ToString();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            Logger.Instance.WriteToLogFile(ConfigurationHelper.ErrorLogProgramm, (string.Format("Инициируется отчет по оплатам для '{0}'", Constants.KleckResLongName)));
            ...
            if ((Session["FromMonth"] != null) && (Session["FromYear"] != null) && (Session["ToMonth"] != null) && (Session["ToYear"] != null))
            {
                ...
                foreach (string tableName in PromTableNamesInterval)
                {
                    ...
                    if (ConvertTablesToOra.Instance.IsTableExistInOra(tableName, ConfigurationHelper.KleckOraConnectionStringBalPoTP))
                    {
            ...
            else if ((Session["Year1"] != null) && (Session["Month1"] != null))
            {
                ...
                if (ConvertTablesToOra.Instance.IsTableExistInOra(promTableName, ConfigurationHelper.KleckOraConnectionStringBalPoTP))
                {
                    promTableNames.Add(promTableName);
                }
                if (ConvertTablesToOra.Instance.IsTableExistInOra(bytTableName, ConfigurationHelper.KleckOraConnectionStringBalPoTP))
                {
            ...
            if ((promTableNames.Count > 0) && (promTableNames.Count <= Constants.PromBytTablesCount))
            {
                ...
                //Получаем список ТП
                List<string> listTpCodes = Balance_Po_VL.OracleOplati.Instance.GetListTPcodes(ConfigurationHelper.KleckPassportConnectionString, Request.QueryString["DOC_CODE"]);

                 
                promTable = Balance_Po_VL.OracleOplati.Instance.GetOplatiTablePromVl(promTableNames, ConfigurationHelper.KleckOraConnectionStringBalPoTP, listTpCodes);
                PromGridView.DataSource = promTable;
                PromGridView.DataBind();
              //  Session["PromTable"] = promTable;
                 
              //  promTable =
              //   Balance_Po_VL.OracleOplati.Instance.PromGetOraTable(Balance_Po_VL.OracleOplati.Instance.FormSqlProm(promTableNames, listTpCodes), ConfigurationHelper.KleckOraConnectionStringBalPoTP, Constants.KleckResLongName);
               // PromGridView.DataSource = promTable;
              //  PromGridView.DataBind();
            ...
            if ((bytTableNames.Count > 0) && (bytTableNames.Count <= Constants.PromBytTablesCount))
            {
                ...
                //Получаем список ТП
                List<string> listTpNames = Balance_Po_VL.OracleOplati.Instance.GetListTPnames(ConfigurationHelper.KleckPassportConnectionString, Request.QueryString["DOC_CODE"]);

                
                bytTable = Balance_Po_VL.OracleOplati.Instance.GetOplatiTableBytVl(bytTableNames, ConfigurationHelper.KleckOraConnectionStringBalPoTP, listTpNames);
                BytGridView.DataSource = bytTable;
                BytGridView.DataBind();
             //   Session["BytTable"] = bytTable;
                  
             //   bytTable =
             //   Balance_Po_VL.OracleOplati.Instance.BytGetOraTable(Balance_Po_VL.OracleOplati.Instance.FormSqlByt(bytTableNames, listTpNames), ConfigurationHelper.KleckOraConnectionStringBalPoTP, Constants.KleckResLongName);
             //   BytGridView.DataSource = bytTable;
             //   BytGridView.DataBind();
            ...
            Logger.Instance.WriteToLogFile(ConfigurationHelper.ErrorLogProgramm, (string.Format("Формирование отчета по оплатам для '{0}' закончено", Constants.KleckResLongName)));
            Logger.Instance.WriteToLogFile(ConfigurationHelper.ErrorLogProgramm, "***********************************************************************************");
        ...
        protected void PromHyper(object sender, EventArgs e)
        {
            ...
            Response.Clear();
            Response.AddHeader("Content-Disposition", "attachment; filename=" + "KleckProm" + Request.QueryString["DOC_NAME_VL10"] + ".xls");
            Response.Charset = "";
        ...
        protected void BytHyper(object sender, EventArgs e)
        {
            ...
            Response.Clear();
            Response.AddHeader("Content-Disposition", "attachment; filename=" + "KleckByt" + Request.QueryString["DOC_NAME_VL10"] + ".xls");
            Response.Charset = "";
            Response.ContentType = "application/vnd.ms-excel";
            Response.Write(sw.ToString());
            Response.Flush();
