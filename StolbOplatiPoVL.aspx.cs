...
namespace Sbyt
{
    public partial class StolbOplatiPoVL : Page
    {
       ...
       // String bytGuid = Guid.NewGuid().ToString();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            Logger.Instance.WriteToLogFile(ConfigurationHelper.ErrorLogProgramm, (string.Format("Инициируется отчет по оплатам для '{0}'", Constants.StolbResLongName)));
            ...
            if ((Session["FromMonth"] != null) && (Session["FromYear"] != null) && (Session["ToMonth"] != null) && (Session["ToYear"] != null))
            {
                ...
                foreach (string tableName in PromTableNamesInterval)
                {
                    ...
                    if (ConvertTablesToOra.Instance.IsTableExistInOra(tableName, ConfigurationHelper.StolbOraConnectionStringBalPoTP))
                    {
            ...
            else if ((Session["Year1"] != null) && (Session["Month1"] != null))
            {
                ...
                if (ConvertTablesToOra.Instance.IsTableExistInOra(promTableName, ConfigurationHelper.StolbOraConnectionStringBalPoTP))
                {
                    promTableNames.Add(promTableName);
                }
                if (ConvertTablesToOra.Instance.IsTableExistInOra(bytTableName, ConfigurationHelper.StolbOraConnectionStringBalPoTP))
                {
            ...
            if ((promTableNames.Count > 0) && (promTableNames.Count <= Constants.PromBytTablesCount))
            {
                ...
                //Получаем список ТП
                List<string> listTpCodes = OracleOplati.Instance.GetListTPcodes(ConfigurationHelper.StolbPassportConnectionString, Request.QueryString["DOC_CODE"]);
                
                promTable = Balance_Po_VL.OracleOplati.Instance.GetOplatiTablePromVl(promTableNames, ConfigurationHelper.StolbOraConnectionStringBalPoTP, listTpCodes);
                PromGridView.DataSource = promTable;
                PromGridView.DataBind();
               // Session["PromTable"] = promTable;
                 
             //   promTable =
            //       OracleOplati.Instance.PromGetOraTable(OracleOplati.Instance.FormSqlProm(promTableNames, listTpCodes), ConfigurationHelper.StolbOraConnectionStringBalPoTP, Constants.StolbResLongName);
              //  PromGridView.DataSource = promTable;
             //   PromGridView.DataBind();
            ...
            if ((bytTableNames.Count > 0) && (bytTableNames.Count <= Constants.PromBytTablesCount))
            {
                ...
                //Получаем список ТП
                List<string> listTpNames = OracleOplati.Instance.GetListTPnames(ConfigurationHelper.StolbPassportConnectionString, Request.QueryString["DOC_CODE"]);

                
                bytTable = Balance_Po_VL.OracleOplati.Instance.GetOplatiTableBytVl(bytTableNames, ConfigurationHelper.StolbOraConnectionStringBalPoTP, listTpNames);
                BytGridView.DataSource = bytTable;
                BytGridView.DataBind();
             //   Session["BytTable"] = bytTable;
               
              //  bytTable =
              //   OracleOplati.Instance.BytGetOraTable(OracleOplati.Instance.FormSqlByt(bytTableNames, listTpNames), ConfigurationHelper.StolbOraConnectionStringBalPoTP, Constants.StolbResLongName);
               // BytGridView.DataSource = bytTable;
               // BytGridView.DataBind();
            ...
            Logger.Instance.WriteToLogFile(ConfigurationHelper.ErrorLogProgramm, (string.Format("Формирование отчета по оплатам для '{0}' закончено", Constants.StolbResLongName)));
            Logger.Instance.WriteToLogFile(ConfigurationHelper.ErrorLogProgramm, "***********************************************************************************");
        ...
        protected void PromHyper(object sender, EventArgs e)
        {
            ...
            Response.Clear();
            Response.AddHeader("Content-Disposition", "attachment; filename=" + "StolbProm" + Request.QueryString["DOC_NAME_VL10"] + ".xls");
            Response.Charset = "";
        ...
        protected void BytHyper(object sender, EventArgs e)
        {
            ...
            Response.Clear();
            Response.AddHeader("Content-Disposition", "attachment; filename=" + "StolbByt" + Request.QueryString["DOC_NAME_VL10"] + ".xls");
            Response.Charset = "";
            Response.ContentType = "application/vnd.ms-excel";
            Response.Write(sw.ToString());
            Response.Flush();
